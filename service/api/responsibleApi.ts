...
export async function createResponsible(data: CreateResponsibleData) {
  return await supabase.from("Responsible").insert(data).throwOnError();
}

export async function updateResponsible(responsibleId: number, data: UpdateResponsibleData) {
  return await supabase.from("Responsible").update(data).eq("id", responsibleId).throwOnError();
}

export async function deleteResponsible(responsibleId: number) {
  return await supabase.from("Responsible").delete().eq("id", responsibleId).throwOnError();
}
