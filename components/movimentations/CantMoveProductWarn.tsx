...
export default function CantMoveProductWarn({ product }: NoProductAmountWarnProps) {
  ...
  if (!hasAmount) {
    msg.push(
      <p className="text-md">
        Não tem <span className="font-bold">nenhuma peça disponível</span> para movimentar.
      </p>,
    );
  }
  ...
  if (!hasDepartament) {
    msg.push(
      <p className="text-md">
        Não definiu <span className="uppercase font-bold">departamento inicial</span>
      </p>,
    );
  }
  ...
  if (!hasProcess) {
    msg.push(
      <p className="text-md">
        Não definiu <span className="uppercase font-bold">processo inicial</span>
      </p>,
    );
  }
    ...
    return (
      <div className="flex flex-col gap-1">
        ...
        <EditProductDialog product={product}>
          <Button className="text-indigo-500 self-start cursor-pointer p-0" variant="link">
            Editar produto
          </Button>
        </EditProductDialog>
      </div>
