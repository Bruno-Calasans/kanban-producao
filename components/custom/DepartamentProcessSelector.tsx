...
export default function DepartamentProcessSelector({
  ...
  const filterProcesses = () => {
    const sortedProcesses = processes.sort(sortBySequence);

    const filteredProcesses = excludeProcess
      ? sortedProcesses.filter(
          (p) => p.id != excludeProcess.id && p.sequence > excludeProcess.sequence,
        )
      : sortedProcesses;
  ...
  const handleValueChange = (processName: string) => {
    const foundProcess = filteredProcesses.find(
      (process) => process.name.toLocaleUpperCase() == processName.toLocaleUpperCase(),
    );
  ...
  return (
    ...
    >
      <SelectTrigger className="min-w-30">
        <SelectValue
          placeholder={processes.length == 0 ? "Nenhum processo" : "Selcione o processo"}
        />
      </SelectTrigger>
