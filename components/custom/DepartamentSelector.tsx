...
export default function DepartamentSelector({
  ...
  const getDepartament = (departamentName: string) => {
    return filteredDepartaments.find(
      (dpt) => dpt.name.toLocaleUpperCase() == departamentName.toLocaleUpperCase(),
    );
  };
    ...
    >
      ...
      <SelectContent position="item-aligned">
        {filteredDepartaments &&
          filteredDepartaments.map((dpt) => (
            <SelectItem key={dpt.id} value={dpt.name}>
              {dpt.name}
            </SelectItem>
          ))}
      </SelectContent>
