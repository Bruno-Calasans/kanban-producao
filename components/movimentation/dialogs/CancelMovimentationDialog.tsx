...
export default function CancelMovimentationDialog({ movimentation, children }: CancelMovimentationDialogProps) {
    ...
    return (
        <CustomDialog
            ...
            <div className="flex flex-col gap-2">
                <p>
                    Tem certeza que deseja cancelar a <strong>movimentação #{movimentation.id}</strong> do produto <strong>{movimentation.product.name}</strong>?
                </p>
                <p>
                ...
                <div className="flex flex-row mt-4 p-2 gap-2 justify-end">
                    <CancelButton hiddenIcon isLoading={isPending} onclick={() => closeDialog("cancel-movimentation")} />
                    <CancelActionButton label="Cancelar movimentação" isLoading={isPending} onclick={handleCancel} />
                </div>
            </div>
        </CustomDialog>
    )
