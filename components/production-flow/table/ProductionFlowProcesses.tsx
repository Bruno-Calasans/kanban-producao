...
export default function ProductionFlowProcesses({ productionFlow }: ProductionFlowProcessesProps) {
  ...
  const canShowExpandButton = processes.length > 2;
  const [expand, setExpand] = useState(false);

  if (isPending) return <Loader title="Carreganddo processos" />;

  return (
    <div>
      <div className={cn("flex flex-col gap-1 overflow-auto max-h-20", expand && " max-h-fit")}>
        {processes.map((process, index) => (
          <>
            <Badge key={process.id + index}>{process.name}</Badge>
          </>
        ))}
      </div>
      {canShowExpandButton && (
